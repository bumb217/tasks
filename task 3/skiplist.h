	...
	else
	{
		...
		for (int l = 0; l < maxLevel; l++)
		{
			while (true)
			{
				...
				while (true)
				{
					while (p->next[currLevel] != nullptr && !p->next[currLevel]->deleted && p->next[currLevel]->val < a)
						p = p->next[currLevel];
					currLevel--;
				...
				while (true)
				{
					while (p1->next[currLevel] != nullptr
				...
				if (p == p1 && !p->deleted) {
					if (p->next[l] == nullptr || p->next[l]->val >= newElem->val)
					{
						newElem->next[l] = next;
...
inline void List<T>::erase(int i)
{
	...
	if (size > i)
	{
		...
		for (int l = 0; l < maxLevel; l++)
		{
			...
			{
				p = this->head;
				for (int t = 1; (t < i && l == 0) || (p->next[l] != nullptr && l>0 && p->next[l] != delItem); t++)
					p = p->next[l];
				auto next = p->next[l];
				if (next == nullptr)
					break;
				shared_ptr<element> p1 = this->head;
				for (int t = 1; (t < i && l == 0) || (p1->next[l] != nullptr && l>0 && p1->next[l] != delItem); t++)
					p1 = p1->next[l];
				if (p == p1) {
					if (l == 0) {
						delItem = next;
					...
					if (atomic_compare_exchange_weak_explicit(&p1->next[l], &p->next[l], next->next[l], memory_order_release, memory_order_relaxed))
					{
						break;
					}
				}
		...
		size--; del = delItem.get(); find(delItem->val);
	}
}
